...
Pada skrip Lua bawaan di dalam CoppeliaSim, algoritma Braitenberg digunakan untuk navigasi penghindaran rintangan sederhana menggunakan pembacaan sensor ultrasonik. Skrip tersebut mendefinisikan kecepatan dasar robot pada variabel berikut:

```lua
v0 = 2
```

Nilai `v0` ini diumpankan secara langsung ke motor roda:
...
$ v = omega times r_w = 2 times 0.0975 = 0.195 "m/s" $

Perhitungan ini membuktikan bahwa kecepatan maju maksimum robot di dalam simulasi sekitar $0.195 "m/s"$.
...
$ V_x = r_w / 2 (omega_R + omega_L) $

Sedangkan kecepatan sudut bodi pusat bergantung pada selisih kecepatan kedua roda yang terdistribusi sepanjang jarak lintasan (_track width_). Jika $r_b$ didefinisikan sebagai setengah dari jarak lintasan total ($L = 2r_b$), maka:
$ omega = r_w / (2 r_b) (omega_R - omega_L) $
...
#figure(
image("odom_real_2.png", width: 80%),
...
Fenomena ini tervalidasi dengan sangat jelas pada _subplot_ kedua (_Temporal Plot of P3DX Body Velocity_). Saat robot bergerak lurus (kecepatan roda konstan), kecepatan linear ($V_x$) terukur berada di garis lurus yang sedikit di bawah $0.2 "m/s"$, yang mana sangat presisi dengan hasil perhitungan analitis kita yaitu $0.195 "m/s"$ pada Persamaan 1.

Selain itu, grafik kecepatan sudut bodi ($omega$) menampilkan empat lembah bernilai negatif tajam yang mencapai titik terendah di sekitar $-0.4 "rad/s"$. Nilai kecepatan sudut yang negatif menegaskan bahwa robot berputar searah jarum jam (_clockwise_). Jumlah empat lembah tersebut mengonfirmasi bahwa robot mendeteksi rintangan dan melakukan manuver putar kanan sebanyak empat kali sepanjang periode observasi 30 detik.
